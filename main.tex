...
Supongamos que tenemos dos objetos, Juan y Pablo, si quisiéramos indicar que Juan es hijo de Pablo podríamos hacerlo mediante el siguiente predicado:
esHijo(Juan,Pablo)
...
En mi caso usé (junto con mis compañeros de equipo en ese trabajo) el lenguaje de programación Java, añadiré el enlace al repositorio por si alguien quiere revisar el código del algoritmo.
...
Antes de explorar algunos de los diferentes algoritmos de aprendizaje vamos a revisar una rutina de optimización que nos permitirá aprender los parámetros $W$ de nuestros modelos. Es por esto que se motiva al lector entender claramente las partes y procesos que forman parte del aprendizaje supervisado presente en la figura \ref{fig:Aprendizaje-supervisado-diagrama}.
...
\item \textbf{Stochastic Gradient Descent (SGD):}: En esta variante del descenso del gradiente se actualiza el valor de los parámetros calculando el gradiente para cada uno de los elementos en el conjunto de datos.
\begin{equation}
...
\item \textbf{Mini-batch Gradient Descent:}: En esta variante del descenso del gradiente se actualiza el valor de los parámetros calculando el gradiente en un subconjunto aleatorio de tamaño m del conjunto de datos.
\begin{equation}
...
\item \textbf{Adagrad}: En esta variante el valor de la tasa de aprendizaje se adapta para cada parámetro basado en el valor histórico del gradiente. Parámetros con un valor grande en el gradiente tienen una tasa de aprendizaje reducida, aquellos con un valor pequeño en el gradiente tienen una tasa de aprendizaje aumentada.

\underline{Ventajas:}
En funciones con parámetros en diferentes escalas ayuda a acelerar la convergencia en un valor optimo.

\underline{Desventajas:}
Debido a que reduce la tasa de aprendizaje con valores grandes en el histórico del gradiente, el aprendizaje puede llegar a detenerse antes antes de converger en un óptimo
\\

\item \textbf{RMSProp}: Al igual que en Adagrad el valor de la tasa de aprendizaja se adapta, sin embargo en lugar de usar el valor histórico del gradiente, usa una ventana en movimiento del promedio del cuadrado de los gradientes
...
A continuación se presenta un enlace a las soluciones en caso de que el lector lo requiera:
...
\begin{itemize}
\item \href {https://www.youtube.com/playlist?list=PLZHQObOWTQDPD3MizzM2xVFitgF8hE_ab}{Lista de reproducción sobre álgebra lineal}
...
Los modelos de apredizaje no supervisado pueden clasificarse de acuerdo a la tarea que pueden realizar:

\begin{itemize}
  \item Clusterización
  \item Reducción de dimensionalidad
  \item Detección de anomalias
  \item Generación de datos
  \item Otras tareas
\end{itemize}

También es posible clasificarlos de acuerdo a si estos modelos se basan en la idea de variables latentes\footnote{Las variables latentes o variables ocultas son aquellas que no observamos pero se pueden inferir a partir de otras variables observables} $Z$. Estos modelos permiten capturar una estructura subyacente en los datos, generalmente de menor dimensionalidad, esto nos puede permitir comprimir los datos al pasar de los datos $X$ a sus variables latentes $Z$ o generar nuevos datos pasando de las variables latentes $Z$ a 
datos $X$ \cite{prince2023understanding}.

\section{Clusterización} \index{Clusterización}

La clusterización consiste en asignar etiquetas a instancias de nuestro conjunto de datos no etiquetados.

\newpage
\textbf{Tipos de clusterización}

Existen diferentes tipos de algoritmos de clusterización a continuación se describen algunos de los más populares \cite{Xu2015}:

\begin{itemize}
  \item \textbf{Algoritmos basados en partición}: La idea básica de estos algoritmos es ubicar el ``centro'' de k clusters en nuestros datos. 
  \item \textbf{Algoritmos basados en densidad}: Este tipo de algoritmos asignan como clusters aquellas áreas con alta densidad de instancias.
  \item \textbf{Algoritmos basados en distribución}: Estos algoritmos parten de la siguiente idea, aquellas instancias generadas a partir de la misma distribución pertenecen al mismo cluster. 
  \item \textbf{Algoritmos basados en jerarquía}: Estos algoritmos construyen una relación jerárquica entre las instancias.
\end{itemize}
...
\begin{figure}[ht]
\centering\includegraphics[width=7.5cm]{ia/k-means clustering.png}
\caption{Clusterización basada en partición utilizando el algoritmo K-means}
\label{fig:kmeans_clustering} 
\end{figure}

\begin{figure}[ht]
\centering\includegraphics[width=7.5cm]{ia/dbscan clustering.png}
\caption{Clusterización basada en densidad utilizando el algoritmo DBSCAN}
\label{fig:dbscan_clustering} 
\end{figure}

\begin{figure}[ht]
\centering\includegraphics[width=7.5cm]{ia/gmm clustering.png}
\caption{Clusterización basada en distribución utilizando el algoritmo Gaussian Mixture Models}
\label{fig:gmm_clustering} 
\end{figure}

\begin{figure}[ht]
\centering\includegraphics[width=7.5cm]{ia/hierarchical clustering.png}
\caption{Clusterización basada en jerarquía}
\label{fig:hierarchical_clustering} 
\end{figure}

\subsection{K - means} \index{K - means}
\label{subsection:k_means}

Este algoritmo basado en partición pretende encontrar $K$ centroides, cada ejemplo del conjunto de datos de entrenamiento pertenece a un centroide por lo cual el resultado son $K$ clusters.

\textbf{Suposiciones del algoritmo}

K-means es un algoritmo de aprendizaje no paramétrico por lo cual no realiza fuertes suposiciones sobre la forma de los clusters, de manera intuitiva se puede entender que este algoritmo asume que los elementos pertenecientes a un mismo cluster poseen características $X$ similares, los centroides se encuentran justo en el centro de las instancias pertenecientes al mismo cluster.

\textbf{Funcionamiento del algoritmo de K-means}

Tendremos los siguientes elementos:

\begin{itemize}
\item  Un set de entrenamiento compuesto de las características $X$.
\item El hiperparámetro $k$ que determina el número de centroides o clusters.
\end{itemize}

El algoritmo es el siguiente:
\begin{enumerate}
\item Se inicializan $k$ centroides $\mathrm{M}$ dentro del espacio de datos $X$, se pueden elegir por ejemplo de forma aleatoria.
\item Se mide la distancia de cada elemento $x$ a cada uno de los clusters (Distintas formas de medir la distancia se detallan en el tema \ref{subsection:knn}).
\item Cada elemento de $X$ es asignado al cluster $\mu$ más cercano.
\item Se actualiza la posición de cada centroide $\mu$ tomando la posición del promedio de los elementos asignados a el.
\item Se repiten los pasos 2, 3 y 4 hasta que se alcance un número máximo de iteraciones o la posición de clusters no se haya modificado en la iteración $t$.
\begin{equation}
\mathrm{M}_{t} = \mathrm{M}_{t-1}
\end{equation}

\end{enumerate}

Matemáticamente podemos entender que estamos minimizando la distancia de los centroides $\mathrm{M}$ a los elementos en $X$ pertenecientes al set $S$ donde cada $x$ fue asignado al cluster $\mu$.

\begin{equation}
J(X,Y,\theta) = cf(X,Y,\theta) + \lambda \sum_{j=1}^{n} | {\theta_{j}} |
\end{equation}

En la Figura \ref{fig:kmeans_clustering} podemos observar un ejemplo del uso del algoritmo K-Means con $k$ igual a 4.
...
\begin{enumerate}
\item En un lenguaje de programación matemático como Octave, Julia o Matlab resolver un problema de clusterización mediante k-means.
\item En cualquier lenguaje de programación utilizar alguna librería como scikit-learn para resolver un problema de clasificación mediante k-means.
\end{enumerate}
